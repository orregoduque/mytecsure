...
def UpdatedDocProfilePk(request, pk):
    ...
    profile = UserProfile.objects.get(user=user)
    if request.method == 'POST':
        form = directorProfileForm(request.POST, instance=profile)
        if form.is_valid():
            form.save()
            return redirect('appointment:hr_dashboard')
    else:
        form = directorProfileForm(instance=profile)
    return render(request, 'user_profile/profile.html', {'form': form, 'user':user})
...
def PerfilListView(request):
    #perfiles = UserProfile.objects.all()
    perfiles = UserProfile.objects.filter(user__user_type="S")
    nombre = 'chiara'

    paginator = Paginator(perfiles, 1) # Show 1 contacts per page.
    ...
    return render(request, 'user_profile/perfiles.html', context)



def OnePerfilListView(request):
    student_id = request.user.get_username()
    estudiante = request.user.id
    
    perfil = UserProfile.objects.get(user=estudiante)

    context = {
        'perfil' : perfil,
    }
    return render(request, 'user_profile/perfil.html', context)